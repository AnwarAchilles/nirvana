...
class Avenirer_Model extends CI_Model
{
    ...
    public function paginate($rows_per_page, $total_rows = NULL, $page_number = 1)
    {
        ...
        $uri_string = implode('/',$uri_array);
        if(isset($total_rows) && (ceil((int)$total_rows/(int)$rows_per_page) == (int)$page_number))
        {
        ...
        if(isset($total_rows))
        {
            if($total_rows!=0)
            {
                $number_of_pages = ceil((int)$total_rows / (int)$rows_per_page);
    ...
    private function _parse_model_dir($foreign_model)
    {
        ...
        if ($full_model) {
            ...
            $model_dir = implode('/', $full_model);
            $data['model_dir'] = str_replace($data['foreign_model'], '', $model_dir);
        }
